...
def show_possible_matches(my_word):
    ...
    return ' '.join(possible_matches_list)
...
def hangman_with_hints(secret_word):
    ...
    * If the guess is the symbol *, print out all words in wordlist that
    ...
    while guesses != 0:
        ...
        guess = input("Please guess a letter: ").lower()
        if guess == '*':
            print(show_possible_matches(guessing_word))
        elif guess not in (string.ascii_letters + string.digits):
            warnings -= 1
            warning_message = "Oops!That is not a valid letter. "
        elif guess in letters_guessed:
            warnings -= 1
        ...
        if warning_message:
            ...
            letters_guessed.append(guess)
            guessing_word = get_guessed_word(secret_word, letters_guessed)
